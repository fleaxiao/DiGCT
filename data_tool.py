...
def extract_label(filename: str):
    ...
    Returns:
        A tuple containing the extracted values: (F, theta, I, angle, Tmax, Tmin).
    """
...
def s2c_angle(image: Image, list: list, Tj: int, k: int, image_size: int) -> Image:
    """
    ...
    for y in range(image_size):
        for x in range(image_size):
            ...
            if distance < radius:
                ...
                if k is None:
                    pixel_value = edge_value
                else:
                    control = min(Tj, edge_value) + (k / 2 + 0.25) * abs(edge_value - Tj)
                    t = radial_ratio
                    pixel_value = int((1 - t) ** 2 * Tj + 2 * (1 - t) * t * control + t ** 2 * edge_value)
...
def empty_inside(image: Image, inner_radius: int) -> Image:
    """
    Set all pixels inside a central circular region to 0.

    Args:
        image: The original image to process.
        inner_radius: Radius (in pixels) of the central area to clear.
    """
    image_size = image.size[0]
    center = image_size // 2
...
def read_t_range(range_file: str, F: int, theta: int, vf: int, I: int) -> tuple:
    """
    ...
    Args:
        ...
        theta: The misplacement angle (degrees) to filter by.
        vf: The velocity value (m/s) to filter by.
        I: The current value (A) to filter by.
    """
    t_range = pd.read_csv(range_file)
    row = t_range[(t_range["F (kN)"] == F) & (t_range["theta"] == theta) & (t_range["vf (m/s)"] == vf) & (t_range["I (A)"] == I)]
    if not row.empty:
        ...
        raise ValueError(f"No matching row for F={F}, theta={theta}, I={I}")

def add_t_range(range_file: str, F: int, theta: int, vf: int, I: int, add_name: str, add_value: float):
    """
    ...
    if not row.empty:
        ...
        raise ValueError(f"No matching row for F={F}, theta={theta}, vf={vf}, I={I}")
