...
class ModelTrainer:
    def __init__(
        ...
        self.physics_constraint = kwargs.get("physics_constraint")
    ...
    def train_epoch(self):
        ...
        for i, (targets, conditions, _, _, _, _, _) in enumerate(pbar):
    ...
    def generate_samples(self, epoch):
        targets, conditions, _, _, _, _, _ = next(cycle(self.test_dataloader))
        targets, conditions = targets.to(self.device), conditions.to(self.device)
        ...
        # generations_pixel = apply_tv_denoising(generations_pixel)

        ssim, _, _, _, mae, _, _ = calculate_metrics(generations_value, targets_value)
    ...
    def train(self):
        ...
        if self.threshold_training == False:
            for epoch in range(self.epochs):
                ...
                if self.ema == True and val_loss < self.best_val_loss:
                    self.best_val_loss = val_loss
                    self.update_best_model(epoch)

                if (epoch + 1) % self.sample_epoch == 0:
                    ssim, mae = self.generate_samples(epoch)
                    logging.info(f"Epoch: {epoch + 1} SSIM: {ssim:.3f} MAE: {mae:.3f}")
