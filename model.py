...
from model_test import sample_show_metrics, show_metrics
...
def main(args):
    ...
    if TRAINING:
        ...
        if GENERATE_SAMPLE == True:
            ...
            print(f"\nSample Model Name: {RUN_NAME}")
            start_sample_time = datetime.datetime.now()
            print(f"Sample Starting Time: {start_sample_time.strftime('%Y-%m-%d %H:%M:%S')}")
            sample_show_metrics(model=model, result_path=RESULT_PATH, device=DEVICE, sampler=trainer.diffusion, length=LENGTH_SAMPLE, test_dataloader=test_dataloader, sample_path=SAMPLE_PATH, **parameters)
            end_sample_time = datetime.datetime.now()
            print(f"Sample Ending Time: {end_sample_time.strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"Sample Duration: {(end_sample_time - start_sample_time).total_seconds() / 60:.2f} minutes")

    if TESTING:
        TEST_PATH = os.path.join(OUTPUT_PATH, TEST_PATH)
        PARAMETER_PATH = os.path.join(TEST_PATH, 'parameters.json')
        MODEL_PATH = os.path.join(os.path.join(TEST_PATH, "model"), SAMPLE_MODEL)
        ...
        for path in [SAMPLE_PATH, TARGET_PATH, GENERATION_PATH, CONDITION_PATH]:
            os.makedirs(path, exist_ok=True)

        dataloader, _, _, _, _, _ = get_data(dataset_path=DATASET_PATH, target_dataset_path=TARGET_DATASET_PATH, condition_dataset_path=CONDITION_DATASET_PATH, result_path=TEST_PATH, split=False, **parameters)

        if CALCULATE_METRICS == True:
            condition_images = load_images(CONDITION_PATH)
            target_images = load_images(TARGET_PATH)
            generation_images = load_images(GENERATION_PATH)
            show_metrics(result_path=TEST_PATH, target_images=target_images[0:1], generation_images=generation_images[0:1], condition_images=condition_images[0:1])
        ...
        if SAMPLE_METRICS == True:
            ...
            sample_show_metrics(model=model, result_path=TEST_PATH, device=DEVICE, sampler=sampler, length=LENGTH_SAMPLE, test_dataloader=dataloader, sample_path=SAMPLE_PATH, **parameters)
...
if __name__ == '__main__':
    ...
    # System settings
    p.add_argument('--physics_constraint', action='store_true', default=False)
    ...
    p.add_argument('--lambda_physics', action='store', type=float, default=0.1)
    ...
    p.add_argument('--length_sample', action='store', type=int, default=1)
